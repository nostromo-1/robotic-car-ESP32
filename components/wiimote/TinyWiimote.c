...
static struct connected_device_t connectedDeviceList[SCANNED_DEVICE_LIST_SIZE];
static int findConnectedDevice(struct bd_addr_t bdAddr) {
  return findItemsInArray((uint8_t*)connectedDeviceList, connectedDeviceListNum, sizeof(struct connected_device_t), (uint8_t*)&bdAddr.addr, BD_ADDR_LEN, 0);
}
...
static struct l2cap_connection_t l2capConnectionList[L2CAP_CONNECTION_LIST_SIZE];
static int l2capFindConnection(uint16_t ch) {
  return findItemsInArray((uint8_t*)l2capConnectionList, l2capConnectionSize, sizeof(struct l2cap_connection_t), (uint8_t*)&ch, sizeof(uint16_t), 0);
}
...
static void handleDisconnectionCompleteEvent(uint8_t len, uint8_t* data) {
    ...
    printf("handleDisconnectionCompleteEvent\n");
...
void handleHciEvent(uint8_t event_code, uint8_t len, uint8_t* data) {
    ...
    if (event_code != HCI_INQUIRY_RESULT_EVT){ // suppress HCI_INQUIRY_RESULT_EVT
      VERBOSE_PRINTLN("EVENT code=%02X len=%d data=%s", event_code, len, format2Hex(data, len));
    }
...
void TinyWiimote_setPlayerLEDs(uint8_t leds) {
   if (wiimoteConnected) setPlayerLEDs(_ch_handle, leds);
}


static void setRumble(uint16_t ch, bool rumble) {
  int idx = l2capFindConnection(ch);
  struct l2cap_connection_t connection = l2capConnectionList[idx];
  ...
  // create information payload of 'Basic information frame'
  // wiimote report: (a2) 10 RR
  uint8_t  posi = 0;
  // Information Payload
  payload[posi++] = 0xA2;  // Output report
  payload[posi++] = 0x10;  // Function:Rumble
  payload[posi++] = rumble?0x01:0x00;
  uint16_t dataLen = posi;
  uint16_t len = make_acl_l2cap_packet(tmpQueueData, ch, pbf, bf, channelID, payload, dataLen);
  sendHciPacket(tmpQueueData, len);
  VERBOSE_PRINT("queued acl_l2cap_single_packet(Set Rumble)");
}


void TinyWiimote_rumble(bool rumble) {
   if (wiimoteConnected) setRumble(_ch_handle, rumble);
}
...
static void setDataReportingMode(uint16_t ch, uint8_t mode, bool continuous) {
  ...
  payload[posi++] = continuous ? 0x04 : 0x00; // TT whether continuous reporting is desired
...
enum {
    ...
    REPORT_STATE_WAIT_READ_CONTROLLER_TYPE,
  ...
  case REPORT_STATE_WAIT_ACK_OUT_REPORT:
    ...
    if((data[1] == 0x22) && (data[4] == 0x16)){
      if(data[5] == 0x00){
        ...
        controllerReportState = REPORT_STATE_WAIT_READ_CONTROLLER_TYPE;
      }else{
        controllerReportState = REPORT_STATE_INIT;
      }
    }
    break;
  case REPORT_STATE_WAIT_READ_CONTROLLER_TYPE:
    VERBOSE_PRINT("REPORT_STATE_WAIT_READ_CONTROLLER_TYPE\n");
    if((data[1] == 0x22) && (data[4] == 0x16)){
      if(data[5] == 0x00){
...
TinyWiimoteData_t receivedData[RECEIVED_DATA_MAX_NUM];

void putWiimoteReceivedData(uint8_t number, uint8_t* data, uint8_t len) {
  if(receivedDataRb.cnt < RECEIVED_DATA_MAX_NUM) {
    TinyWiimoteData_t *target = &(receivedData[receivedDataRb.wp]);
    memcpy(target->data, data, len);
    target->number = number;
    target->len = len;
    receivedDataRb.wp = (receivedDataRb.wp + 1) % RECEIVED_DATA_MAX_NUM;
    receivedDataRb.cnt++;
  }
  VERBOSE_PRINTLN(" ");
}
...
TinyWiimoteData_t TinyWiimoteRead() {
  TinyWiimoteData_t target;
  target.number = 0;
  target.len = 0;
  if(receivedDataRb.cnt > 0) {
    target = receivedData[receivedDataRb.rp];
    receivedDataRb.rp = (receivedDataRb.rp + 1) % RECEIVED_DATA_MAX_NUM;
    receivedDataRb.cnt--;
  }
  return target;
}

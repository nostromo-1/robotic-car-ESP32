...
static const float odr_ag_modes[] = {0,14.9,59.5,119,238,476,952};

static const enum {M_ODR_0_625,M_ODR_1_25,M_ODR_2_5,M_ODR_5,M_ODR_10,M_ODR_20,M_ODR_40,M_ODR_80} ODR_M = M_ODR_40; 
static const float odr_m_modes[] = {0.625,1.25,2.5,5,10,20,40,80};
...
esp_err_t rc;
int delay;
static const uint8_t command = 0x18;
   ...
   for (int i=0; i<samples; i++) {
      ...
      rc = i2c_master_write_read_device(I2C_BUS, accelAddr, &command, 1, buf, sizeof(buf), I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
...
static int i2cWriteByteData(uint8_t addr, uint8_t reg, uint8_t val)
{
esp_err_t rc;
uint8_t buf[2];
...
// Send a byte command to the IMU and read a byte as response
static int i2cWriteReadByteData(uint8_t addr, uint8_t comm, uint8_t *ret_val)
{
esp_err_t rc;
          
    rc = i2c_master_write_read_device(I2C_BUS, addr, &comm, 1, ret_val, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
...
uint8_t byte;
   ...
   *delay = 1+lroundf(1000.0/odr_m_modes[ODR_M]);  // Read IMU with a period of magnetometer ODR
   
   // Check acelerometer/gyroscope   
   rc = i2cWriteReadByteData(accel_addr, 0x0F, &byte);  // Read WHO_AM_I register
   ...
   // Check magnetometer  
   rc = i2cWriteReadByteData(mag_addr, 0x0F, &byte);  // Read WHO_AM_I register
   if (rc < 0) goto rw_error;
   if (byte != 0x3D) {
      //closeLSM9DS1();
   ...
   rc = i2cWriteByteData(magAddr, 0x20, byte);
   if (rc < 0) goto rw_error;  
   ...
   byte = 0x0; 
   rc = i2cWriteByteData(magAddr, 0x21, byte);
   if (rc < 0) goto rw_error;   
   ...
   rc = i2cWriteByteData(accelAddr, 0x10, byte);
   if (rc < 0) goto rw_error; 
   ...
   byte = 0x02; 
   rc = i2cWriteByteData(accelAddr, 0x11, byte);
   if (rc < 0) goto rw_error; 
   ...
   rc = i2cWriteByteData(accelAddr, 0x12, byte);
   if (rc < 0) goto rw_error;  

   /* Empty and reset FIFO, in case it was active, so we start afresh */
   rc = i2cWriteByteData(accelAddr, 0x2E, 0x00);  // Reset existing FIFO content by selecting FIFO Bypass mode 
   if (rc < 0) goto rw_error;  
   ...
   */
   ...
   byte = (0x06<<5) + 0x0; 
   rc = i2cWriteByteData(accelAddr, 0x2E, byte);
   if (rc < 0) goto rw_error; 
   ...
   byte = 0x02; 
   rc = i2cWriteByteData(accelAddr, 0x23, byte);
   if (rc < 0) goto rw_error;  
